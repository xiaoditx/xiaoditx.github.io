...
> [!note]
> 
> `restrict`关键字是C语言关键字，C++标准并不支持这一关键字，不过有些C++编译器可能会支持`__restrict`，不过这就会因编译器不同而有所差异了
...
> [!note]
> 
> 智能指针是C++特有的内容，如果你只想学习C语言，请直接跳过本条到[2.常量指针、指针常量](#2常量指针指针常量)

> [!tip]
> 
> 接下来的内容会占据很长的篇幅，各位一定要有耐心
...
> [!warning]
> 
> 当你获取到原始指针时，一定不要试图去delete，我们在[后面的内容中](#17释放资源)会论述为什么这样做是危险的
...
shared_ptr有一些特有的操作，常用的是`use_count()`，它可以查看某个shared_ptr指向的内容的引用计数，也就是同时有多少shared_ptr在指向这个内存
...
C++20标准之前，shared_ptr还有一个`unique()`命令，不过在20以后就弃用了，这个命令可以检查智能指针是否是指向的内存的唯一所有者
...
智能指针是为了解决内存泄漏而生，但依旧没有完全规避掉内存泄漏，比如循环引用依旧会导致内存泄漏发生

循环引用的发生通常需要两个类，并且各自有一个或多个指向对方的`shared_ptr`指针：
...
我们可以说数组是指针的变种，那么自然数组也可以退化为指针，在很多情况下，数组会自动退化为指针：
